...
def compute_k(focal_length, principal_point, height=513, width=513):
    fx = focal_length[0]
    fy = focal_length[1]
...
def train_epoch(config, epoch, model1, model2, criterion, optimizer, scheduler, train_loader, smpl_model):
    losses=[]
    model1.train()
    model2.train()
    ...
    for idx, data in tqdm(enumerate(train_loader)):
        ...
        faces = faces.clone().detach().requires_grad_(True)
        print('img',img.shape)
        ...
        texture_feature = model1(img).to(config.device)
        _, _, img_h, img_w = img.shape
...
def eval_epoch(config, epoch, model1, model2, criterion, test_loader, smpl_model):
    with torch.no_grad():
        losses = []
        model1.eval()
        model2.eval()
        ...
        for idx, data in tqdm(enumerate(test_loader)):
            ...
            faces = torch.from_numpy(faces_numpy)
            # faces = torch.tensor(faces, dtype=torch.int64).to(config.device)
            faces = faces.clone().cpu().numpy()
            print('img', img.shape)

            verts, face, aux = load_obj(obj_file[0])
            verts = verts.to(config.device)
            # verts = torch.tensor(verts, dtype=torch.float)
            verts = verts.clone().cpu().numpy()
