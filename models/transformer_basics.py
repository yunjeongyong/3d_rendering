...
class TransformerEncoderUnitSparse(nn.Module):
    ...
    def forward(self, src):
        ...
        src2 = self.attn1(q=src + pos_embed, k=src + pos_embed, v=src, attn_type='sparse_long', ah=self.ahw[0],
                          aw=self.ahw[1])[0]
        src = src + src2  # ? this might be ok to remove

        # multi-head short-range attention
        src2 = self.attn2(q=src + pos_embed, k=src + pos_embed, v=src, attn_type='sparse_short', ah=self.ahw[2],
                          aw=self.ahw[3])[0]
        src = src + src2
        src = self.norm1(src)
...
class TransformerDecoderUnitSparse(nn.Module):
    def __init__(self, feat_dim, n_head=8, pos_en_flag=True, ahw=None):
        ...
        self.attn2_1 = OurMultiheadAttention(feat_dim,
                                             n_head)  # cross-attention: self-attention-long + cross-attention-short
    ...
    def forward(self, tgt, src):
        ...
        tgt2 = \
        self.attn1_1(q=tgt + tgt_pos_embed, k=tgt + tgt_pos_embed, v=tgt, attn_type='sparse_long', ah=self.ahw[0],
                     aw=self.ahw[1])[0]
        tgt = tgt + tgt2
        # self-multi-head attention: sparse short
        tgt2 = \
        self.attn1_2(q=tgt + tgt_pos_embed, k=tgt + tgt_pos_embed, v=tgt, attn_type='sparse_short', ah=self.ahw[2],
                     aw=self.ahw[3])[0]
        tgt = tgt + tgt2
        tgt = self.norm1(tgt)
        ...
        # self-multi-head attention: sparse long
        src2 = \
        self.attn2_1(q=src + src_pos_embed, k=src + src_pos_embed, v=src, attn_type='sparse_long', ah=self.ahw[4],
                     aw=self.ahw[5])[0]
        src = src + src2
        # cross-multi-head attention: sparse short
        tgt2 = \
        self.attn2_2(q=tgt + tgt_pos_embed, k=src + src_pos_embed, v=src, attn_type='sparse_short', ah=self.ahw[6],
                     aw=self.ahw[7])[0]
        tgt = tgt + tgt2
        tgt = self.norm2(tgt)
