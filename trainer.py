...
# create model
model1 = UNet(n_channels=3, n_classes=8, bilinear=False).to(config.device)
model2 = UNet(n_channels=8, n_classes=3, bilinear=False).to(config.device)
...
for epoch in range(start_epoch, config.n_epoch):
    loss = train_epoch(config, epoch, model1, model2, criterion, optimizer, scheduler, train_loader, smpl_model,ras_sett)

    if (epoch + 1) % config.val_freq == 0:
        loss = eval_epoch(config, epoch, model1, model2, criterion, test_loader, smpl_model, ras_sett)
