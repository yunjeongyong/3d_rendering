...
def train_epoch(config, epoch, model1, model2, criterion_m, criterion_p, optimizer, scheduler, train_loader, smpl_model, raster):
    losses=[]
    model1.train()
    model2.train()
    ...
    for idx, data in tqdm(enumerate(train_loader)):
        ...
        loss_total = loss_mse + loss_perceptual
...
def eval_epoch(config, epoch, model1, model2, criterion_m, criterion_p, test_loader, smpl_model, raster):
    with torch.no_grad():
        losses = []
        model1.eval()
        model2.eval()
