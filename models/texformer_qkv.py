...
from models.transformers.net_utils import single_conv, double_conv, double_conv_down, double_conv_up, PosEnSine
from models.transformers.transformer_basics import OurMultiheadAttention
...
class Texformer_qkv(nn.Module):
    def __init__(self, opts):
        ...
        if not self.mask_fusion:
            ...
            self.conv6 = nn.Sequential(single_conv(self.feat_dim, self.feat_dim),
                                       nn.Conv2d(self.feat_dim, 2 + 3 + 1, 3, 1,
    ...
    def forward(self, q, k, v):
        ...
        f5 = f5 + outputs[2] + f0
        if not self.mask_fusion:
            out = self.sigmoid(self.conv6(f5))
        else:
            out_ = self.conv6(f5)
            out = [self.sigmoid(out_[:, :2]), self.sigmoid(out_[:, 2:5]), self.sigmoid(out_[:, 5:])]
        return out
