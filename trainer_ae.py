...
from traincode_test import train_epoch, eval_epoch
from models.smpl import SMPL
from common import constants
from models.autoencoder import YoonNet
import pickle
import numpy as np
...
model1 = YoonNet(config=config, ngf=64, dim=512, a=3).to(config.device)
model2 = YoonNet(config=config, ngf=64, dim=512, a=4).to(config.device)
...
for epoch in range(start_epoch, config.n_epoch):
    loss = train_epoch(config, epoch, model1, model2, criterion, optimizer, scheduler, train_loader, smpl_model)
    # print('[train] epoch:%d / loss:%f / SROCC:%4f / PLCC:%4f' % (epoch+1, loss.item(), rho_s, rho_p))

    if (epoch + 1) % config.val_freq == 0:
        loss = eval_epoch(config, epoch, model1, model2, criterion, test_loader, smpl_model)
